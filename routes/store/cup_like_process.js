...
router.post('/store/like_process/cup', function (request, response) {
    ...
    request.on('end', function(){
        ...
        db.query(`SELECT * FROM liked WHERE store_id=? and user_id=? and type=?`, [id,user,2], function(err0,res0){
            if(res0.length == 0){
                db.query(`INSERT INTO liked(store_id, user_id, type) VALUES(?, ?, ?)`, [id, user, 2], function(err1, res1){
                    if(err1) throw err1;
                    response.writeHead(302, {Location: `/store/cup/1`});
                    response.end();
                });
            } else{
