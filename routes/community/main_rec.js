...
router.get('/community/recommendation/list/:listId', function (request, response) {
    db.query(`SELECT * FROM recommendation`, function(err, count){
        ...
        db.query(`SELECT * FROM recommendation LEFT JOIN user ON recommendation.writer = user.email LIMIT ?, ?`, [offset, list_size], function(err, res){
            var title = '제품추천';
            var head = `
                <style>
                table, td ,th {
            ...
            for(i = first; i < last + 1; i++){
                paging += `
                    <li class="page-item"><a class="page-link" href="/community/recommendation/list/${i}">${i}</a></li>
                `
            }
            var body = `
